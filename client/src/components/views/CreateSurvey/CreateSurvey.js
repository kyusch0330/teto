...
import CreateSurveyBody from "../../CreateSurveyBody/CreateSurveyBody";
  ...
  return (
    <div>
      <Formik
        ...
        onSubmit={(values) => {
          setFixedTypes(values.types);
        }}
        render={({ values }) => <CreateTypes types={values.types} />}
      />
      {fixedTypes.length > 0 && <CreateSurveyBody types={fixedTypes} />}
    </div>
  );
};
export default CreateSurvey;
